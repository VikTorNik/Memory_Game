document.addEventListener("DOMContentLoaded", () => {
    ...
    showHeader = () => {
        ...
        currentDiv.innerHTML =
            "<p>Memory Game - за мотивами «Лісової пісні» Лесі Українки</p>";
    ...
    createObjectCardImage = () => {
        ...
        [...Array(200)].map(() =>
            randomArray.add(`img/${Math.floor(1 + Math.random() * 28)}.jpg`)
        );
        let baseCardImage = [
            ...Array(Math.floor(DATE_GAME.gridLevel.countGrid / 2)),
        ].map((_, i) => [...randomArray.values()][i]);

        baseCardImage = [...baseCardImage, ...baseCardImage].sort(
            () => Math.random() - 0.5
        );

        for (key in DATE_GAME.cardFace) {
            delete DATE_GAME.cardFace[key];
        }

        baseCardImage.map((pathImage, idImage) => {
            const currentCard = (DATE_GAME.cardFace[`card-${idImage + 1}`] =
                new Object());
    ...
    showCurrentLevel = () => {
        ...
        for (let card of Object.values(DATE_GAME.cardFace)) {
            fieldPlay.push(
                `<div class="div__card"><img id=${card.id} class=${card.statusClass}`
            );
            card.statusClass == "card-back"
                ? fieldPlay.push(` src=${DATE_GAME.cardBack.pathImage}`)
                : card.statusClass == "card-face"
                ? fieldPlay.push(` src=${card.pathImage}`)
                : card.statusClass == "card-checked"
                ? fieldPlay.push(` src=${card.pathImage}`)
                : card.statusClass == "card-off-checked"
                ? fieldPlay.push(` src=${DATE_GAME.cardBack.pathImage}`)
                : false;
    ...
    checkAllGuessedCards = () => {
        if (
            Object.values(DATE_GAME.cardFace).every(
                (card) => card.statusClass == "card-checked"
            )
        ) {
            Object.values(DATE_GAME.cardFace).map(
                (card) => (card.statusClass = "card-face")
            );
        }
    };
    ...
    showFooter = () => {
        const currentDiv = document.createElement("div");
        currentDiv.innerHTML =
            '<p>2022 • Memory Game by <a class="git" href="https://github.com/VikTorNik" target="_blank">VikTorNik</a>  • Автор ляльок, лялькової постановки та фотограф <a class="git" href="https://www.facebook.com/people/Olena-Nekora/100050930892384/" target="_blank">Olena Nekora</a></p>';
